...
// LOADS THE MAP ONLOAD AND DYNAMICALLY APPENDS THE TEXT AREA, CALLS THE
// geoCode() method
function loadmap() {
...
// geoCode() METHOD GETS THE LATITUDE AND LONGITUDE onclick event AND PASSES THE
// COORDINATES TO REVERSE CODING FUNCTION
function rgeoCode() {
			...
			if (status == google.maps.GeocoderStatus.OK) {
				if (results[1]) {
					...
					try {
						...
						// alert("after");
						request.open("GET", "proxy.php?zws-id=" + zwsid
...
// APPENDS THE TEXTAREA WITH ZESTIMATE AMOUNT AND INFO PLACES THE MARKER WITH
// INFOWINDOW LOADED WITH SAME DETAILS
function displayResultr() {
	...
	if (request.readyState == 4) {
		...
		try {
			var value3 = xml.getElementsByTagName("zestimate")[0]
					.getElementsByTagName("amount")[0].childNodes[0].nodeValue;
			code = xml.getElementsByTagName("code")[0].childNodes[0].nodeValue;

			// ONLY IF THE CODE == 0 WHICH IMPLIES SUCCESSFUL VALUE IN THE API
			// RESPONSE
			if (code == '0' && value3 != null || value3 == '0' || value3 == '') {
				...
				infowindow.setContent("<font color='red'>" + "$" + value3
						+ "</font>" + ":" + full_adr);
				infowindow.open(map, marker);

				document.getElementById("text_display").value = document
						.getElementById("text_display").value
						+ "\n"
						+ "AMOUNT:$"
						+ value3
						+ "\n"
...
// APPENDS THE VALUE AND LOADS THE MARKER WITH ZESTIMATE AND OTHER INFO TO
// IMPLEMENT THE GEOCODING FUNCTION
function displayResult() {
	if (request.readyState == 4) {
		var xml = request.responseXML.documentElement;
		// var value =
		// xml.getElementsByTagName("zestimate")[0].getElementsByTagName("amount")[0];
		try {
			var value2 = xml.getElementsByTagName("zestimate")[0]
					.getElementsByTagName("amount")[0].childNodes[0].nodeValue;
			code = xml.getElementsByTagName("code")[0].childNodes[0].nodeValue;
			// alert(code);
			if (code == '0' && value2 != null) {
				...
				document.getElementById("text_display").value = document
						...
						+ "\n"
						+ "STATE:"
						+ xml.getElementsByTagName("state")[0].childNodes[0].nodeValue
						+ "\n"
						+ "ZIPCODE:"
						+ xml.getElementsByTagName("zipcode")[0].childNodes[0].nodeValue
						+ "\n";
						...
						.geocode(
								...
								function(results, status) {
									if (status == google.maps.GeocoderStatus.OK) {
										...
										infowindow1
												.setContent("<font color='red'>"
														+ "$"
														+ value2
														+ "</font>"
														+ ":"
														+ xml
																.getElementsByTagName("street")[0].childNodes[0].nodeValue
														+ ":"
														+ xml
														...
														+ "ZIPCODE:"
														+ xml
																.getElementsByTagName("zipcode")[0].childNodes[0].nodeValue);
...
function sendRequest() {
	...
	var url = "proxy.php?zws-id=" + zwsid + "&address=" + address
			+ "&citystatezip=" + city + "+" + state + "+" + zipcode;
	var encoded_url = encodeURI(url);

	request.open("GET", encoded_url);
	request.withCredentials = "true";
	request.send(null);
	// geoCode();
}